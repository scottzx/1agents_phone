...
/// Classifies user-role rows that were generated by another agent/runtime,
/// rather than typed by the human. They keep the user role for model-history
/// alternation, but the UI may safely render their Markdown because their
/// content came from an assistant or a structured group transcript.
public enum AgentInboundMessageClassifier {
    /// Stable, locale-independent marker prepended to direct A2A envelopes.
    public static let directRelayMarker = "📨 "

    public static func shouldRenderMarkdown(_ text: String) -> Bool {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        return GroupChatPrompt.isGroupTurnPrompt(trimmed)
            || trimmed.hasPrefix(directRelayMarker)
    }
}
